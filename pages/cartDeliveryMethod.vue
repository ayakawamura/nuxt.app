...
// import DeliveryMethodComp from '~/components/cart/DeliveryMethodComp.vue';
import type { DeliveryAddress } from '~/types/cart';
import { getDeliveryAddressList } from '~/composables/cart/getApi'


const deliveryAddressList = ref<DeliveryAddress[]>(getDeliveryAddressList())

deliveryAddressList.value.forEach((address) => {
  watch(
    () => address.deliveryMethod.name,
    (newVal) => {
      if (newVal === 'メール便') {
        address.deliveryMethod.hopeDate = null
        address.deliveryMethod.hopeTime = null
      }
    },
    // { immediate: true } // ← 初期化時にも実行したいなら追加
  )
})
...
<template>
  <div id="sysMain">
    ...
    <div v-for="deliveryAddress in deliveryAddressList" :key="deliveryAddress.id">
      <!-- <DeliveryMethodComp :deliveryAddress=deliveryAddress /> -->
      <div class="main-container">
        <div class="sub-container1">
          <div class="deliveryCount">{{ deliveryAddress.id + 1 }}件目</div>
          <div class="deliveryName">{{ deliveryAddress.name }}様
            <div class="deliveryAddress">{{ deliveryAddress.postalCode }} {{ deliveryAddress.address }}</div>
          </div>
          <div>
            <p>配送方法：{{ deliveryAddress.deliveryMethod.name }}</p>
            <p>配送希望日：{{ deliveryAddress.deliveryMethod.hopeDate }}</p>
            <p>配送希望時間帯：{{ deliveryAddress.deliveryMethod.hopeTime }}</p>
          </div>
        </div>
        ...
        <div class="sub-container2">
          ...
          <details class="shipWrap accordion-006">
            <summary>
              <span>出荷{{ deliveryAddress.id + 1 }}<span class="shipItems">対象の商品はこちら</span></span>
            </summary>
            <div v-for="item in deliveryAddress.item" :key="item.id" class="item-image">
              <div>
                <img class="opcItem_itemImage" :src="item.url" title="画像をクリックして詳細ページへ"
                  alt="十二単「満開」(お米2合×12個)">
          ...
          <div class="deliveryMethod">
            <div class="deliveryName">
              <input id="deliveryAddress.id" type="radio" :name="'method_' + deliveryAddress.id"
                v-model="deliveryAddress.deliveryMethod.name" value="宅配便" checked="true">
              <label for="deliveryAddress.id">宅配便</label>
            </div>
            <div class="deliveryMethodSelect" v-show="deliveryAddress.deliveryMethod.name == '宅配便'">
              <div class="opcDeliveryMethod_normalShippingDetailAndLabelGroup_edit">
                <div class="opcDeliveryMethod_hopeDeliveryDateAndLabelGroup_edit">
                  ...
                  <div>
                    <select v-model="deliveryAddress.deliveryMethod.hopeDate"
                ...
                <div class="opcDeliveryMethod_hopeDeliveryTimeAndLabelGroup_edit">
                  ...
                  <select v-model="deliveryAddress.deliveryMethod.hopeTime"
            ...
            <div class="deliveryComent" v-show="deliveryAddress.deliveryMethod.name == '宅配便'">
          ...
          <div class="deliveryMethod">
            <div class="deliveryName">
              <input type="radio" v-model="deliveryAddress.deliveryMethod.name" :name="'method_' + deliveryAddress.id"
                value="メール便">メール便
            </div>

            <div class="deliveryComent" v-show="deliveryAddress.deliveryMethod.name == 'メール便'">
              <details class="accordion-006">
                <summary>配送コメント</summary>
                <div>ここに配送コメント</div>
              </details>
          ...
          <div class="deliveryMethod">
            <div class="deliveryName">
              <input type="radio" :name="'method_' + deliveryAddress.id" v-model="deliveryAddress.deliveryMethod.name"
                value="店舗受取">店舗受取
            </div>
            <div class="deliveryMethodSelect" v-show="deliveryAddress.deliveryMethod.name == '店舗受取'">
              <div class="opcDeliveryMethod_normalShippingDetailAndLabelGroup_edit">
                <div class="opcDeliveryMethod_hopeDeliveryDateAndLabelGroup_edit">
            ...
            <div class="deliveryComent" v-show="deliveryAddress.deliveryMethod.name == '店舗受取'">
              <details class="accordion-006">
    ...
    <CompleteButton nextButtonText="次へ（お支払い方法の設定）" link="/cartPayment" />
    <BackButton link="/cartDeliveryAddress" />
